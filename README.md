...
I browsed it for a while and at the very start we see lots of requests 10.0.2.74 (the attacker presumably) to 10.0.2.75, this looks like a typical port scan. I figured that for the first question the easy way is to filter like this: 
...
Why did is answer in the second packet, not first one (they both send the encrypted cipher) - turns out that is because the latter packet is etype 23 which is crackable and used for AS-REP roasting, while etype 18 is not crackable (AES256), so the attacker captured an easy to crack short hash.
...
We have the structure that we need to make and after adjustments the hash becomes: 
